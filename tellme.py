...
from discord import Guild, Member, Message, Reaction, Role, TextChannel, VoiceChannel
...
if not discord.opus.is_loaded():
  raise ModuleNotFoundError("Tried to load opus but failed, so TellMe.py is unable to run", name="opus")
...
class TellMe(commands.Cog):
  ...
  async def play(self, ctx: Context):
    ...
    for _round in range(min(1, self.rounds)):
      ...
      for _turn, player in enumerate(self.players):
        ...
        if self.turn == 0 and self.round == 0:
          s = await self.say(
            ctx,
            msg=f"Tell me a {self.genre} story set in {self.location} with {'' if self.item[-1] == 's' else 'an' if self.item[0] in 'aeiouh' else 'a'} {self.item}",
          )
        ...
        s = await self.say(
          ctx,
          msg=f"You will have {time} seconds to tell me a story. You should hear an alert when there are ten seconds remaining. Your {time} seconds starts, now.",
        )
        self.audio_files.append(s)
    ...
    with open(c, "w+") as w:
      w.write("\n".join([f"file '{audio_file.parent.name!s}/{audio_file.name}'" for audio_file in self.audio_files]))
      w.write("\n")
    u = Path(f"./audio/session-{datetime.now():%Y-%m-%d-%H-%M-%S}.m4a")
    run(["ffmpeg", "-f", "concat", "-safe", "0", "-loglevel", "panic", "-i", c, u], check=False)
  ...
  async def say(self, ctx: Context, msg: str):
    if msg[:12] == "./audio/pre/":
      ...
      gTTS(msg).save(str(m))
      run(["ffmpeg", "-loglevel", "panic", "-i", m, f], check=False)
      # m.unlink()
    wait = (
      float(
        json.loads(
          run(
            ["ffprobe", "-i", f, "-loglevel", "quiet", "-print_format", "json", "-show_streams"],
            encoding="utf-8",
            stdout=PIPE,
            check=False,
          ).stdout
  ...
  async def record(self, ctx: Context, resample=True):
    ...
    time = max(15, int(self.T))
    print(f"Recording {time}s in {recording!s}")
    ...
    self.audio_files.append(s)
    print(), print(), print(), print()
    print(f"Recording complete {recording!s}")
    print(), print(), print(), print()
    return recording

  async def vote(self, ctx: Context, keywords=None):
    if keywords is not None and keywords != [] and keywords != self.keywords:
    ...
    message: Message
    reactions = {}
    for k, (n, e) in zip(self.keywords, enumerate(["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"]), strict=False):
    ...
    self.prompts = [
      self.keywords[reactions[react.emoji]] for react in sorted(reacts, key=lambda react: react.count, reverse=True)[:4]
    ]
    print("Voted for:")
    print(self.prompts)
